...
INDEX_PLAIN = os.path.join(PROJECT_ROOT, "data", "index.json")  # fallback if needed



def text_to_keywords(text: str) -> list[str]:
    """
    Convert search query to a list of clean keywords.
    Similar to caption_to_keywords used when building keywords for captions.
    Steps:
    - lowercase
    - remove punctuation
    - split into words
