...
# Configure robust downloads before importing transformers
...
try:
    ...
    logger_temp.warning("huggingface_hub not found. Robust downloads disabled.")
...
def load_model_safe(model_class, model_name, cache_dir, **kwargs):
    """
    Try loading from local cache first. If failed, download from hub robustly.
    """
    try:
        logger.info(f"Loading {model_name} from local cache (trying original format)...")
        ...
        try:
            logger.info(f"Trying to load {model_name} from snapshot directory...")
            return model_class.from_pretrained(
                cache_dir,
                local_files_only=True,
                **kwargs
            ...
            logger.info(f"Local cache not found for {model_name}. Downloading robustly...")
            if snapshot_download:
                try:
                    ignore_list = ["*.h5", "*.msgpack", "*.ot", "onnx/*", "coreml/*"]
                    # BLIP has both .bin and .safetensors (1.88GB each). We only need safetensors.
                    if "blip" in model_name.lower():
                        ignore_list.append("*.bin")
                    # CLIP only has .bin, so we don't ignore it.
                ...
                logger.info("Falling back to standard transformers download...")
                return model_class.from_pretrained(
                    model_name,
                    cache_dir=cache_dir,
